...
Sphere::Sphere(Vecteur3 center_, float radius_, shared_ptr<Material> mat_ptr_):center(center_),radius(radius_),mat_ptr(mat_ptr_)
{}

bool Sphere::hit(const Ray &r, float t_min, float t_max, hit_record &h_rec) const{
    Vecteur3 oc = r.getOrigin() - center;
    Vecteur3 ray_direct = r.getDirect();
    ...
    if(hit_at < t_max && hit_at > t_min){
        ...
        Vecteur3 normal_out = (h_rec.p - center).normalize();
